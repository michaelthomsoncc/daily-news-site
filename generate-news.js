...
async function generateNews() {
  ...
  const storiesPrompt = `You are a gaming, tech, and world news curator for a sharp UK gamer. Generate exactly 20 unique stories based strictly on well-researched, factually accurate current events from your up-to-date knowledge as of ${today}. Do not invent, fabricate, or speculate—only use verified facts from real news happening today. Balance topics: ~5 on new game updates/releases (patches, betas, launches), ~6 on PC hardware (GPUs, controllers, keyboards, builds), ~5 on major world events (wars, global crises—focus on factual updates/impacts), ~4 on UK government actions (policies on tech/gaming/education that could shift things, like app regs or taxes).
...
CRITICAL: Before generating, use your real-time web_search tool to verify current events from today (${today}). Only include stories with confirmed sources. 
...
MANDATORY: Start by calling web_search or browse_page on "${story.source}" for today's facts (query: "${story.title} ${today}"). Base EVERY detail on results and real facts.".
Structure:
- Hook: 1 para.
...
Output clean HTML only: <p> paras, <strong> emphasis, <em> quotes. 400-600 words. No <h1> or title repeat.`;
    ...
    try {
      const storyResponse = await openai.chat.completions.create({
        ...
        max_tokens: 2500, // Bumped for tool outputs
      });
      ...
      // Enhanced fallback: Fact-check prompt as placeholder
      const factCheckPrompt = `Fact-check this story title and summary against real news today (${today}): "${story.title}". ${story.summary}. Output ONLY a short HTML para: If verified, "<p>Verified facts incoming—check back.</p>"; else "<p>Unverified: Skipping for accuracy.</p>".`;
      try {
        ...
        let storyHtml = storyTemplate
          ...
          .replace(/\{timestamp\}/g, timestamp);
        fs.writeFileSync(filePath, storyHtml);
        console.log(`Fallback fact-check for ${story.title.substring(0, 30)}...`);
      } catch (fallbackError) {
        // Basic placeholder
